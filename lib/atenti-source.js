/**
 * lib/atenti-source.js — Helpers para leer los logs crudos de Atenti desde
 * el repo privado `atenti-logs-raw` en GitHub. Usado por api/atenti-search.js.
 *
 * Los logs los sube Apps Script (atenti-github-push.gs) automáticamente cada
 * día, con acceso a Gmail sin OAuth que expire. Vercel solo necesita un token
 * de GitHub (sin OAuth, sin refresh_token).
 *
 * Env: GITHUB_LOGS_TOKEN — PAT con read access al repo atenti-logs-raw
 *      GITHUB_LOGS_REPO  — opcional, default: gastoncarreecommerce/atenti-logs-raw
 */

const LOG_FILES = [
  { key: "chatLog",            name: "chat.log"            },
  { key: "loginLog",           name: "login.log"           },
  { key: "agregarLog",         name: "agregar.log"         },
  { key: "categorizarLog",     name: "categorizar.log"     },
  { key: "buscarEansLog",      name: "buscar_eans.log"     },
  { key: "buscarSimilaresLog", name: "buscar_similares.log"},
  { key: "origenLog",          name: "origen.log"          },
];

export async function fetchLogsForDate(date) {
  const token = process.env.GITHUB_LOGS_TOKEN || process.env.GITHUB_TOKEN;
  const repo  = process.env.GITHUB_LOGS_REPO  || "gastoncarreecommerce/atenti-logs-raw";

  if (!token) throw new Error("Falta GITHUB_LOGS_TOKEN en Vercel env vars");
  ...
  const result = {};
  for (const { key, name } of LOG_FILES) {
    const url = `https://api.github.com/repos/${repo}/contents/${date}/${name}`;
    const res = await fetch(url, { headers });
    if (res.ok) {
      const data = await res.json();
      result[key] = Buffer.from(data.content.replace(/\n/g, ""), "base64").toString("utf-8");
    }
  }

  if (!result.chatLog) return null;
  return result;
}
...
// Todos los carritos agregados ese día, agrupados por DNI (excluye "0" =
// sesión anónima/sin login).
export function getAllCartAdds(agregarLog) {
  const entries = parseEntriesWithTs(agregarLog);
  const byDni = new Map();
  for (const e of entries) {
    const m = e.text.match(/^(\d+): Agregando al carrito: /);
    ...
    const json = extractJson(e.text, m[0].length);
    if (!json) continue;
    const items = (json.items || []).filter(i => i?.name).map(i => ({ nombre: i.name, cantidad: Number(i.quantity) || 1 }));
    if (!items.length) continue;
    if (!byDni.has(dni)) byDni.set(dni, []);
    byDni.get(dni).push(...items);
  }
  return byDni;
}
...
// búsquedas, pedidos de receta y agregados al carrito, todo en orden cronológico.

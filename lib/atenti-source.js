 ...
 * por fecha puntual. Usado SOLO por api/atenti-search.js.
 *
 * Los logs se obtienen vía Google Apps Script (apps-script/atenti-logs.gs),
 * que corre autenticado como el dueño de la cuenta Gmail — sin OAuth externo
 * ni tokens que vencer semanalmente.
 *
 * Env requeridas: APPSCRIPT_URL, APPSCRIPT_SECRET
 */
...
export async function fetchLogsForDate(date) {
  ...
  if (!APPSCRIPT_URL || !APPSCRIPT_SECRET)
    throw new Error("Faltan APPSCRIPT_URL / APPSCRIPT_SECRET en Vercel");

  const url = `${APPSCRIPT_URL}?key=${encodeURIComponent(APPSCRIPT_SECRET)}&date=${encodeURIComponent(date)}`;
  ...
  return {
    ...
    agregarLog:     data.agregarLog     || "",
    categorizarLog: data.categorizarLog || "",
    buscarEansLog:  data.buscarEansLog  || ""
  };
}
...
// sesión anónima/sin login). Para la estadística agregada de atribución de
// ventas: necesitamos saber, de TODOS los DNIs que metieron algo al carrito
// sugerido por Atenti, no solo de uno puntual.

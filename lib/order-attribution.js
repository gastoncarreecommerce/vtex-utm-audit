...
export async function getAttributionStats(cartAddsByDni, date, days = 7) {
  ...
  const stats = { usuarios_con_sugerencia: dnis.length, compraron: 0, compraron_con_sugerido: 0 };
  if (!dnis.length) return stats;

  const results = await mapLimit(dnis, 5, async dni => {
    try {
      return await findAttributedOrder(dni, date, cartAddsByDni.get(dni), days);
    } catch {
      return null;
    }
  });
  ...
  for (const r of results) {
    if (!r) continue;
    stats.compraron++;
    if (r.productos_sugeridos_comprados.length) stats.compraron_con_sugerido++;
  }
  return stats;
}
